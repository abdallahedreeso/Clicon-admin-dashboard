        ...
        categories.forEach(category => {
            ...
            row.innerHTML = `
                ...
                <td class="p-4 flex space-x-2">
                    <button class="bg-yellow-500 text-white p-2 rounded" onclick="editCategory(${category.category_id})">Edit</button>
                    <button class="bg-red-600 text-white p-2 rounded" onclick="openConfirmationModal(${category.category_id})">Delete</button>
                </td>
            `;
...
async function handleFormSubmit(event) {
    ...
    if (category_id) {
        await editCategoryData({ category_id, category, sub_category, description });
    } else {
...
document.getElementById('openCategoryModalBtn').addEventListener('click', window.openAddCategoryModal);
document.getElementById('closeCategoryModalBtn').addEventListener('click', () => document.getElementById('categoryModal').classList.add('hidden'));
document.getElementById('cancelDeleteBtn').addEventListener('click', window.closeConfirmationModal);
document.getElementById('confirmDeleteBtn').addEventListener('click', window.deleteCategory);
