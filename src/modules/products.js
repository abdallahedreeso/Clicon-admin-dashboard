...
async function fetchProducts() {
  ...
  products.forEach((product) => {
    ...
    row.innerHTML = `
            <td class="p-4 w-fit">${product.name}</td>
            ...
            <td class=" p-4 lg:flex lg:space-x-2">
                <button class="bg-yellow-500 text-white w-full mb-2 lg:mb-0 p-2 rounded" onclick="editProduct(${product.product_id})">Edit</button>
                <button class="bg-red-600 text-white w-full p-2 rounded" onclick="openConfirmationModal(${product.product_id})">Delete</button>
            </td>
        `;
...
async function editProductData(productData) {
  const { productId, name, description, price, category_id, quantity } =
    productData;
  try {
    const { error } = await supabaseClient
      .from("products")
...
document
  ...
  .addEventListener("click", window.openAddProductModal);
document
...
document
  ...
  .addEventListener("click", window.deleteProduct);
document
